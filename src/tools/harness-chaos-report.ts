...
interface ExperimentData {
  ...
  createdAt: string;
  updatedAt?: string;
  infra?: { identity?: string; environmentId?: string };
  recentWorkflowRunDetails?: RunDetail[];
}
...
interface ExperimentReport {
  ...
  stoppedRuns: number;
  errorRuns: number;
  avgResiliencyScore: number | null;
  bestScore: number | null;
  worstScore: number | null;
  scoreTrend: "improving" | "degrading" | "stable" | "insufficient_data";
  runs: Array<{
    runId: string;
    sequence: number;
    phase: string;
...
async function fetchExperiments(
  ...
  const resp = raw as { data?: ExperimentData[] };
  return resp?.data ?? [];
}
...
function msToDate(ms: string | number): Date {
...
function analyzeExperiment(
  ...
  if (runsInWindow.length === 0) return null;
  ...
  let scoreTrend: ExperimentReport["scoreTrend"] = "insufficient_data";
  if (scores.length >= 3) {
    ...
    else if (diff < -5) scoreTrend = "degrading";
...
async function generatePdf(
  ...
  const allInsights = reports.flatMap((r) =>
    r.insights.map((insight) => ({ experiment: r.experimentName, insight })),
  );
  const degrading = reports.filter((r) => r.scoreTrend === "degrading");
  const needsAttention = reports.filter(
    (r) => r.completedRuns === 0 && r.totalRuns > 0,
  );
  ...
  if (overallAvg === null) { grade = "N/A"; gradeColor = GRAY; }
  else if (overallAvg >= 90) { grade = "A"; gradeColor = GREEN; }
  else if (overallAvg >= 70) { grade = "B"; gradeColor = GREEN; }
  else if (overallAvg >= 50) { grade = "C"; gradeColor = ORANGE; }
  else if (overallAvg >= 30) { grade = "D"; gradeColor = ORANGE; }
  ...
  if (needsAttention.length > 0) {
    recommendations.push(
      `CRITICAL: ${needsAttention.length} experiment(s) had runs but none completed — ` +
      `investigate: ${needsAttention.map((r) => r.experimentName).join(", ")}`,
    );
  }
  if (degrading.length > 0) {
    recommendations.push(
      `WARNING: ${degrading.length} experiment(s) show degrading resiliency scores — ` +
      `check for recent regressions: ${degrading.map((r) => r.experimentName).join(", ")}`,
    );
  }
  if (overallAvg !== null && overallAvg < 50) {
    recommendations.push(
      "Overall resiliency is below 50% — prioritize adding retry logic, circuit breakers, and graceful degradation patterns.",
    );
  }
  if (totalStopped > totalCompleted) {
    recommendations.push(
      `More runs were stopped (${totalStopped}) than completed (${totalCompleted}) — review ChaosGuard rules and infrastructure stability.`,
    );
  }
  if (totalErrors > 0) {
    recommendations.push(
      `${totalErrors} run(s) errored out — check chaos infrastructure connectivity and agent health.`,
    );
  }
  if (reports.length === 0) {
    recommendations.push(
      "No experiments were executed in this timeframe. Schedule regular chaos experiments to continuously validate resilience.",
    );
  }
  if (recommendations.length === 0) {
    recommendations.push(
      "All experiments are performing well. Continue running chaos experiments regularly and expand fault coverage.",
    );
  }
...
export function registerChaosReportTool(server: McpServer, client: HarnessClient, config: Config): void {
  server.registerTool(
    "harness_chaos_report",
    {
      description:
        "Generate a PDF resilience report for a Chaos project. " +
      ...
      try {
        ...
        if (!args.start_date?.trim() || !args.end_date?.trim()) {
          return jsonResult({
            ...
            message:
              "Ask the user for the report timeframe. Both start_date and end_date are required.",
            prompts: [
        ...
        const outputPath = resolve(
          args.output_path ?? `./chaos-report-${projectId}-${args.start_date}-to-${args.end_date}.pdf`,
        );
        ...
        const totalRuns = reports.reduce((s, r) => s + r.totalRuns, 0);
        const allScores = reports.map((r) => r.avgResiliencyScore).filter((s): s is number => s !== null);
        const overallAvg = allScores.length > 0
          ? Math.round((allScores.reduce((a, b) => a + b, 0) / allScores.length) * 10) / 10
          : null;
        const allInsights = reports.flatMap((r) =>
          r.insights.map((i) => ({ experiment: r.experimentName, insight: i })),
        );
