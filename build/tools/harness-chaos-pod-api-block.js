...
async function listEnvironments(client, accountId, orgId, projectId) {
...
async function listInfrastructures(client, accountId, orgId, projectId, envId) {
    const raw = await client.request({
        method: "GET",
...
export function registerPodApiBlockTool(server, client, config) {
    server.registerTool("harness_chaos_pod_api_block", {
        description: "Create a Pod API Block chaos experiment for testing downstream dependency, " +
        ...
        try {
            ...
            if (!args.environment_id) {
                ...
                if (envs.length === 0) {
                    return errorResult(`No environments found in project "${projectId}" (org: "${orgId}"). ` +
                        "Create an environment first in the Harness UI or via harness_chaos_onboard.");
                }
                return jsonResult({
                    status: "environments_listed",
                    message: "Ask the user to choose an environment from the list below, then " +
                        "call harness_chaos_pod_api_block again with environment_id set.",
                    project_id: projectId,
                    org_id: orgId,
                    environments: envs,
                    next_step: 'Call harness_chaos_pod_api_block with environment_id="<chosen_id>" ' +
            ...
            if (!args.infra_id) {
                ...
                const infras = await listInfrastructures(client, config.HARNESS_ACCOUNT_ID, orgId, projectId, args.environment_id);
                if (infras.length === 0) {
                    return errorResult(`No infrastructures found in environment "${args.environment_id}" ` +
                        `(project: "${projectId}", org: "${orgId}"). ` +
                        "Create a Kubernetes infrastructure first.");
                }
                return jsonResult({
                    status: "infrastructures_listed",
                    message: "Present the infrastructure list to the user and ask them to choose one. " +
                    ...
                    runtime_inputs_required: {
                        description: "Ask the user to provide ALL of the following. These are mandatory runtime inputs — " +
                    ...
                    next_step: "After the user provides the infrastructure choice AND all 4 runtime inputs above, " +
            ...
            if (missingInputs.length > 0) {
                return jsonResult({
                    ...
                    message: "The following runtime inputs are required from the user before the experiment can be created. " +
            ...
            const deepLink = `${config.HARNESS_BASE_URL}/ng/account/${config.HARNESS_ACCOUNT_ID}` +
...
//# sourceMappingURL=harness-chaos-pod-api-block.js.map