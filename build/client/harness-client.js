...
function humanizeHttpError(status, rawBody) {
 ...
 */
function summarizeRequestForTimeoutLog(options, timeoutMs) {
    ...
    if (typeof options.body === "object" &&
        ...
        typeof options.body.query === "string") {
        const b = options.body;
        const graphql = {
            queryLength: b.query.length,
            query: b.query.length > MAX_TIMEOUT_LOG_GRAPHQL
...
export class HarnessClient {
    ...
    async request(options) {
        ...
        const chaosPath = options.path.includes("/chaos/");
        const ccmPath = options.path.includes("/ccm/") || options.path.includes("/lw/");
        const sessionPath = chaosPath || ccmPath;
        if (sessionPath && this.cookie) {
            headers["Cookie"] = this.cookie;
        }
        else if (sessionPath && this.bearerToken) {
            headers["Authorization"] = `Bearer ${this.bearerToken}`;
        }
        else {
            if (!this.apiKey) {
                throw new HarnessApiError("HARNESS_API_KEY is required. Set it as a PAT (pat.<accountId>.<tokenId>.<secret>) or service account token.", 400);
            }
        ...
        for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
            ...
            try {
                ...
                try {
                    ...
                    throw new HarnessApiError(`Non-JSON response from ${method} ${path}: ${text.slice(0, 200)}`, 502, undefined, undefined, parseErr);
                }
                log.info("Harness API response", {
                    outcome: "success",
                    method,
