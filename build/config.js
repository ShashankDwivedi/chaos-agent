...
export const ConfigSchema = RawConfigSchema.transform((data) => {
    ...
    const accountId = data.HARNESS_ACCOUNT_ID?.trim() || (apiKey ? extractAccountIdFromToken(apiKey) : undefined);
    if (!accountId) {
        throw new Error("HARNESS_ACCOUNT_ID is required when HARNESS_API_KEY is missing or not a PAT (pat.<accountId>.<tokenId>.<secret>). Set it explicitly when using HARNESS_BEARER_TOKEN or HARNESS_COOKIE only.");
    }
