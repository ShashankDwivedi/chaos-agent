...
function mcpJsonRpcMethodFromBody(body) {
...
function createHarnessServer(config) {
    ...
    const server = new McpServer({
        ...
        instructions: "IMPORTANT: Call harness_chaos_guide (no parameters) at the start of every " +
...
async function main() {
    process.on("unhandledRejection", (reason) => {
        log.error("Unhandled promise rejection — exiting", { error: String(reason), stack: reason?.stack });
        process.exit(1);
    });
